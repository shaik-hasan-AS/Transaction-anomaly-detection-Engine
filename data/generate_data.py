        ...
        customers.append(
            {
                ...
                "home_lat": city["lat"]
                + random.uniform(-0.1, 0.1),  # slight jitter around city center
...
def generate_transactions(
    ...
    account_to_customer = {
        a["account_id"]: next(
            c for c in customers if c["customer_id"] == a["customer_id"]
        )
        for a in accounts
    }
    ...
    account_devices = {
        a["account_id"]: random.sample(devices, random.randint(1, 2)) for a in accounts
    }
    ...
    for _ in range(num_days * tx_per_day):
        ...
        hour = max(
            6, min(23, hour)
        )  # Force 6 AM to 11 PM to avoid accidental Late Night anomalies
    ...
    for tx_time in timestamps:
        ...
        favored_merchants = [
            m for m in local_merchants if m["category"] in profile_data["categories"]
        ]
        ...
        status = (
            "DECLINED"
            if random.random() < profile_data["decline_chance"]
            else "SUCCESS"
        )
...
def inject_anomalies(transactions, customers, accounts, cards, merchants, devices):
    ...
    for _ in range(50):
        ...
        # Tx 1: New York
        ny_merchant = next(
            (m for m in merchants if m["country"] == "USA"), merchants[0]
        )
        transactions.append(
            {
                "transaction_id": str(uuid.uuid4()),
        ...
        tokyo_merchant = next(
            (m for m in merchants if m["country"] == "Japan"), merchants[1]
        )
        transactions.append(
            {
                "transaction_id": str(uuid.uuid4()),
    ...
    for _ in range(50):
        ...
        customer_id = next(
            a["customer_id"] for a in accounts if a["account_id"] == account_id
        )
        customer_country = next(
            c["country"] for c in customers if c["customer_id"] == customer_id
        )
...
def main():
    ...
    transactions = generate_transactions(
        customers, accounts, cards, merchants, devices, start_date
    )
    transactions = inject_anomalies(
        transactions, customers, accounts, cards, merchants, devices
    )

    print("Clearing old data...")
    cur.execute(
        "TRUNCATE TABLE transactions, devices, merchants, cards, accounts, customers CASCADE;"
    )

    print("Inserting data...")

    cur.executemany(
        "INSERT INTO customers (customer_id, name, country, created_at) VALUES (%s, %s, %s, %s)",
        [
            (c["customer_id"], c["name"], c["country"], c["created_at"])
            for c in customers
        ],
    )

    cur.executemany(
        "INSERT INTO accounts (account_id, customer_id, account_type, created_at, status) VALUES (%s, %s, %s, %s, %s)",
        [
            (
                a["account_id"],
                a["customer_id"],
    ...
    cur.executemany(
        "INSERT INTO cards (card_id, account_id, card_type, created_at, status) VALUES (%s, %s, %s, %s, %s)",
        [
            (
                c["card_id"],
                c["account_id"],
    ...
    cur.executemany(
        "INSERT INTO merchants (merchant_id, merchant_name, category, latitude, longitude, country) VALUES (%s, %s, %s, %s, %s, %s)",
        [
            (
                m["merchant_id"],
                m["merchant_name"],
    ...
    cur.executemany(
        "INSERT INTO devices (device_id, device_type, operating_system, created_at) VALUES (%s, %s, %s, %s)",
        [
            (d["device_id"], d["device_type"], d["operating_system"], d["created_at"])
            for d in devices
        ],
    )
    ...
    conn.close()
    print(
        f"Successfully generated and inserted {len(transactions)} realistic transactions."
    )
